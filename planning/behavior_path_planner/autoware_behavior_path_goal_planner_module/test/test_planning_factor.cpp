  ...
  const std::string & yaml_file, std::vector<std::string> corrupted_check_list = {})
{
  const auto yaml_path = autoware::test_utils::get_absolute_path_to_test_data(
    "autoware_behavior_path_goal_planner_module", yaml_file);
...
public:
  ...
  void setupForStartBlocked()
  {
    ...
    test_manager_->publishInput(
      ...
      autoware::test_utils::makeMapBinMsg("autoware_test_utils", "road_shoulder/lanelet2_map.osm"));

    const std::string output_planning_factors_topic = "planning/planning_factors/goal_planner";
    ...
    // NOTE: this test repeats inputs several times because the GoalPlannerModule has a delay
    // between setting the goal and publishing the path and planning factors.
...
TEST_F(PlanningFactorTest, GoalPlannerBlocked)
{
