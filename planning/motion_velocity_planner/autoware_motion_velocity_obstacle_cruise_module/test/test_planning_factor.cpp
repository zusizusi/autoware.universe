  ...
  void SetUp() override
  {
    ...
    std::vector<std::string> yaml_files = {
      ...
      ament_index_cpp::get_package_share_directory(
        ...
        "/config/obstacle_cruise.param.yaml"};
    autoware::test_utils::updateNodeOptions(node_options, yaml_files);

    node_ = std::make_shared<rclcpp::Node>("test_node", node_options);
    module_ = std::make_unique<autoware::motion_velocity_planner::ObstacleCruiseModule>();

    module_->init(*node_, "obstacle_cruise");
  }

  void TearDown() override { rclcpp::shutdown(); }

  std::unique_ptr<autoware::motion_velocity_planner::ObstacleCruiseModule> module_;
...
TEST_F(PlanningFactorTest, TestWithPredictedObjects)
{
  ...
  EXPECT_EQ(
    planning_factor.behavior, autoware_internal_planning_msgs::msg::PlanningFactor::UNKNOWN);
