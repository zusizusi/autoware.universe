...
def generate_test_description():
    ...
    launch_file = os.path.join(
        get_package_share_directory("autoware_object_range_splitter"),
        "launch",
        "object_range_splitter.launch.xml",
    )
...
class TestLaunchComposable(unittest.TestCase):
    ...
    def test_node_exists(self):
        ...
        while time.time() < end_time and not node_found:
            ...
            if "object_range_splitter" in node_names:
                node_found = True
            else:
                rclpy.spin_once(self.test_node, timeout_sec=0.1)

        self.assertTrue(node_found, "object_range_splitter node not found in container")
