...
__device__ __half ms_deform_attn_im2col_bilinear<__half>(
  ...
  int32_t const & channels, const __half & h, const __half & w, int32_t const & m,
  int32_t const & c)
{
...
__global__ void ms_deformable_im2col_gpu_kernel<__half>(
  ...
  int32_t const * dataLevelStartIndex, const __half * dataSamplingLoc,
  const __half * dataAttnWeight, int32_t const batchSize, int32_t const spatialSize,
  int32_t const numHeads, int32_t const channels, int32_t const numLevels, int32_t const numQuery,
  int32_t const numPoint, __half * dataCol)
{
  CUDA_1D_KERNEL_LOOP(index, n)
  {
    int32_t _temp = index;
    ...
    for (int32_t lCol = 0; lCol < numLevels; ++lCol) {
      ...
      for (int32_t pCol = 0; pCol < numPoint; ++pCol) {
        ...
        if (
          __hgt(hIm, kMINUS_ONE) && __hgt(wIm, kMINUS_ONE) && __hlt(hIm, spatialHHalf) &&
...
#else
        const __half hIm = __float2half(
          __half2float(locH) * __half2float(spatialHHalf) - __half2float(kZERO_POINT_FIVE));
        const __half wIm = __float2half(
          __half2float(locW) * __half2float(spatialWHalf) - __half2float(kZERO_POINT_FIVE));

        if (
          (__half2float(hIm) > __half2float(kMINUS_ONE)) &&
